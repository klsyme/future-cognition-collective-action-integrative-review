...
uthm <- ut_authors_combined

# ---- assumptions ----
# uthm: your data.frame / tibble
# UTid : unique per-row analysis identifier (required)
stopifnot("UTid" %in% names(uthm))
...
one_hot_single_col <- function(df, id_col = "UTid", field) {
...
oh_mediators   <- one_hot_single_col(uthm, field = "Mediators")
oh_moderators  <- one_hot_single_col(uthm, field = "Moderators")
oh_predictors  <- one_hot_single_col(uthm, field = "Predictors")
oh_dvk         <- one_hot_single_col(uthm, field = "dv.k2")

# NEW (optional): one-hot for Data Collection Methods
oh_dcm         <- one_hot_single_col(uthm, field = "Data Collection Methods")

# ---- helper: safe join that OR-collapses duplicate indicator columns ----
safe_join_or <- function(x, y, key = "UTid") {
...
onehot_all <- uthm %>% select(UTid)

# Join in any order; duplicates will be collapsed to a single column name
onehot_all <- safe_join_or(onehot_all, oh_moderators, key = "UTid")
onehot_all <- safe_join_or(onehot_all, oh_mediators,  key = "UTid")
onehot_all <- safe_join_or(onehot_all, oh_predictors, key = "UTid")
onehot_all <- safe_join_or(onehot_all, oh_dvk,        key = "UTid")

# NEW (optional): include the one-hot expansion of Data Collection Methods
onehot_all <- safe_join_or(onehot_all, oh_dcm,        key = "UTid")

# ---- replace any remaining NAs with 0 across indicator columns ----
indicator_cols <- setdiff(names(onehot_all), "UTid")
...
onehot_all <- onehot_all %>%
  left_join(
    uthm %>% select(UTid, Citation, `Data Collection Methods`),
    by = "UTid"
  ) %>%
  mutate(
  ...
  relocate(Citation, `Data Collection Methods`, .after = UTid)

# ---- quick verification for an example row (edit "ut11" if needed) ----
# onehot_all %>%
#   filter(utid == "ut11") %>%
#   select(UTid, Citation, `Data Collection Methods`)
