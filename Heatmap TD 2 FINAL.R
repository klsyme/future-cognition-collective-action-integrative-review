...
stopifnot(
  "TDid" %in% names(onehot_all),
  "Data Collection Methods" %in% names(onehot_all)
)

stopifnot(!anyDuplicated(onehot_all$TDid))
...
meta_cols <- c("TDid", "Data Collection Methods")
...
rownames(M) <- onehot_all2$TDid
X <- t(M)

storage.mode(X) <- "numeric"
X <- ifelse(is.na(X), NA_real_, ifelse(X >= 0.5, 1, 0))

sub_df <- onehot_all2[, c("TDid", "Data Collection Methods")]
...
ann_col <- sub_df %>%
  select(TDid, `Data Collection Methods`) %>%
  column_to_rownames("TDid")
...
ph <- pheatmap(
  ...
  fontsize = 10,
  main = sprintf("Variables × TDid — Jaccard‑ordered (k = %d)", k_clusters),
  color = binary_colors,
...
png("tdid_heatmap.png", width = 5000, height = 6000, res = 450)
