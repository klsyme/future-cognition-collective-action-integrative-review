...
stopifnot(
  "AEid" %in% names(onehot_all),
  "Data Collection Methods" %in% names(onehot_all)
)

stopifnot(!anyDuplicated(onehot_all$AEid))

# Remove Citation to match model script
...
meta_cols <- c("AEid", "Data Collection Methods")
...
rownames(M) <- onehot_all2$AEid
X <- t(M)

storage.mode(X) <- "numeric"
X <- ifelse(is.na(X), NA_real_, ifelse(X >= 0.5, 1, 0))

sub_df <- onehot_all2[, c("AEid", "Data Collection Methods")]
...
ann_col <- sub_df %>%
  select(AEid, `Data Collection Methods`) %>%
  column_to_rownames("AEid")
...
ph <- pheatmap(
  ...
  fontsize = 10,
  main = sprintf("Variables × AEid — Jaccard‑ordered (k = %d)", k_clusters),
  color = binary_colors,
...
png("aeid_heatmap.png", width = 5000, height = 6000, res = 450)
...
row_cluster_df <- data.frame(
  Variable   = names(row_clusters),
  RowCluster = as.integer(row_clusters),
  row.names  = NULL
)

col_clusters <- if (!is.null(col_hc)) {
  cutree(col_hc, k = min(k_clusters, length(cols_keep)))
} else NULL

col_cluster_df <- if (!is.null(col_clusters)) {
  data.frame(
    AEid       = names(col_clusters),
    ColCluster = as.integer(col_clusters),
    row.names  = NULL
  )
} else NULL
